...
### Sound on frequency, cycles, period, wavelength
Consider frequencies as the resource in which the sound exist.
Each frequency is a resource, and the sound is the unit.
They don't compete for space, you have the spectrum and two frequencies can exist at the same time and space.
...
The relations for $$f$$ with relate to $$T$$ and $$\lambda$$ are cases of pure orthopolity,
in which the medium in which they exist does not impose restrictions for them to coexist (nor competition mechanics).
...
The volume of the sound is not considered here, but could, in many ways.
...
Deviations are observed to respect a constant in the rank variable: $$f \propto (r + b)^{-\alpha}$$ (with $$\alpha$$ is typically $$\approx 1$$), the Zipf-Mandelbrot law, and also are found to produce quasi-Zipfian distributions is some time-dependant corpora.
...
Is power law an evidence of orthopolity or $$I$$ and $$\Psi(I)$$ don't relate
