...
One of these things to be observed is that in real systems the medium imposes restrictions: the power law distorts in many ways.
...
not considered in the spectrum of $$c * [r(u)] ^ {-\alpha}$$.
...
The postulate 1) is a natural law, and the deviations from it are the result of other natural laws.
The apple can get stuck in the tree after you throw it up: gravity is acting and it does not fall,
i.e. natural law of gravitation is acting even if we don't measure the apple falling down.
...
This translates to more things of that size may exist in your house or in your city, for example.
See the picture for things size propability from 0.01m to 5m in a box with $$l=20m$$:

Now lets understand why it is not perfectly a power-law distribution in the case of your body or house or your city.

Let's say you need a fraction of the volume empty, say $$\epsilon$$.
Supose you now have boxes of length $$l_i$$ and will fit them in the greater box,
and these boxes cannot be put inside one another.
Now there are many ways we can think on fitting the boxes in the greater box.
Lets iterate over the box sizes and fill the free space with them.
You can interate from the smallest to the largest box, or from the largest to the smallest box.
Next you see the picture for both cases:

### Sound on cycles
Consider frequencies as the resource in which the sound exist.
Each frequency is a resource, and the sound is the unit.
They don't compete for space, you have the spectrum and two frequencies can exist at the same problem.
You can think about it a special case of the boxes, in which they exist in one dimension.

Let's consider time first.
The larger the period (the resource), the less cycles it has (less frequent, less numerous it is):
$$f = 1 / T$$
...
The relations for $f$ with relact to $$T$$ and $$\lambda$$ are cases of pure orthopolity,
in which the medium in which they exist does not impose restrictions for them to compete or coexist.

Sound generated by real systems often present constraints, such as harmonic spectrum,
resonance features, etc.
Thus, observed frequency of sound spectra is valid in boundaries and deviate from the power law.
The volume of the sound is not considered here, but could.

Maybe if a frequency is so loud as to have enough energy to "break the medium" or change its behavior,
then the orthopolity of sound on cycles is broken and the power law is distorted.
...
where $$f$$ is the frequency of the word, $$r$$ is the rank of the word, and $$\alpha$$ is typically in $$[1, 2]$$.
Deviations are observed to respect a constant in the rank variable: $$f \propto (r + b)^{-\alpha}$$, the Zipf-Mandelbrot law, and also are found to produce quasi-Zipfian distributions is some time-dependant corpora.

### Steven's law (and Weber-Fechner law)
The intensity of the stimulus $$I$$ is related to the magnitude of is sensation $$Psi(I)$$ through:
$$\Psi(I) = k * I^{-\alpha}$$
which is a power law with $$\alpha$$ typically in $$[-2, -0.3]$$.
Is this a case of orthopolite of the power law or $$I$$ and $$\Psi(I)$$ don't relate
as "quantities in basquets?
...
Consider that person $$u_i$$ spends an hour in one activity/subject $$r_j$$,
and we shall register it as an hour of knowledge about activity $$r_j$$.
...
For example one knowledge influences the other (if you know a lot about civil engineering, you might know a lot about architecture, for example).
Also the number of hours a person have is limited, life changes and so the activities, etc.

You might observe that related to knowledge, as expected by the power law,
you have very few leading references (hubs),
a number of specialists (intermediaries),
and a lot (a lot) of people that know a little about the subject (periferals).
...
The number of resources held by people with $$k$$ resources is:
$$Q(k) = N * k^{-\alpha} * p(k) = N * C = C'$$ which is constant.
Considering $$k$$ discrete, one can see that total resources allocated to people with $$k$$ resources is constant.
...
### Uniform(k) => Power(k)
One of the most impressive consequences of the orthopolity principle is that the uniform distribution of resources  yields a power law distribution of resources.
...
The power law distribution is that of resources among containers, which can be expressed with $$p(k) = C * k^{-\alpha}$$. The first equation is the orthopolity principle, the second is the natural distribution law.
